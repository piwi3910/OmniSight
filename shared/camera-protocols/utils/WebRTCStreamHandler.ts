...
export class WebRTCStreamHandler extends EventEmitter {
  ...
  private setupSignalingEvents(): void {
    ...
    this.signalingServer.on(SignalingServerEvent.STREAM_REQUESTED, 
      this.handleStreamRequest.bind(this));
      
    // Handle connection established
    this.signalingServer.on(SignalingServerEvent.CONNECTION_ESTABLISHED,
      this.handleConnectionEstablished.bind(this));
      
    // Handle connection closed
    this.signalingServer.on(SignalingServerEvent.CONNECTION_CLOSED,
      this.handleConnectionClosed.bind(this));
      
    // Handle client disconnected
    this.signalingServer.on(SignalingServerEvent.CLIENT_DISCONNECTED,
      this.handleClientDisconnected.bind(this));
      
    // Handle errors
    this.signalingServer.on(SignalingServerEvent.ERROR,
      this.handleSignalingError.bind(this));
  }
   ...
   */
  private handleStreamRequest(event: any): void {
    ...
    const connectionId = this.signalingServer.createStreamConnection(streamId, {
      encoding: options.encoding || stream.config.encoding || 'h264',
      resolution: options.resolution || stream.config.resolution,
      frameRate: options.frameRate || stream.config.frameRate,
      maxBitrate: options.maxBitrate || stream.config.maxBitrate
    });
   ...
   */
  private handleConnectionEstablished(event: any): void {
   ...
   */
  private handleConnectionClosed(event: any): void {
    const { connectionId } = event;
   ...
   */
  private handleClientDisconnected(event: any): void {
   ...
   */
  private handleSignalingError(event: any): void {
    const { error } = event;
