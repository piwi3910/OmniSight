...
export class NATTraversalHelper extends EventEmitter {
  ...
  private async checkStunServerReachability(server: string): Promise<boolean> {
    const serverHost = server.replace(/^stun[s]?:/, '');
  ...
  private async checkTurnServerReachability(server: { 
    ...
    const results = await Promise.all(
      urls.map(async url => {
        const serverHost = url.replace(/^turn[s]?:/, '');
  ...
  public recordSelectedCandidatePair(local: string, remote: string): void {
    ...
    if (localInfo && remoteInfo) {
      ...
      this.stats.relayRequired = 
        localInfo.type === ICECandidateType.RELAY || 
        remoteInfo.type === ICECandidateType.RELAY;
   ...
   */
  public generateIceConfiguration(): RTCConfiguration {
    return {
    ...
    } as any; // Type cast required due to TypeScript's RTCConfiguration differences
  }
