...
export class HLSProtocol extends AbstractCameraProtocol {
  ...
  private setupEventHandlers(): void {
    ...
    this.eventEmitter.on(HLSProtocolEvent.SEGMENT_CREATED, (data: any) => {
    ...
    this.eventEmitter.on(HLSProtocolEvent.STREAM_STARTED, (data: any) => {
      this.stats.activeStreams = this.hlsStreams.size;
    });
    
    // Handle stream stopped events
    this.eventEmitter.on(HLSProtocolEvent.STREAM_STOPPED, (data: any) => {
      this.stats.activeStreams = this.hlsStreams.size;
    });
    
    // Handle error events
    this.eventEmitter.on(HLSProtocolEvent.ERROR, (error: any) => {
  ...
  protected async performDisconnect(): Promise<void> {
    ...
    for (const [streamId, stream] of this.hlsStreams.entries()) {
  ...
  async getFrame(): Promise<Uint8Array> {
    ...
    try {
      ...
      this.logger.error(`Error extracting frame: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  protected async performStartStream(options?: StreamOptions): Promise<string> {
    ...
    try {
      ...
      const selectedQualities = options?.profile 
        ? [this.defaultStreamSettings.videoQualities?.find(q => q.name === options.profile)].filter(Boolean) 
      ...
      const streamSettings: HLSStreamSettings = {
        segmentDuration: options?.parameters?.segmentDuration as number || this.defaultStreamSettings.segmentDuration || 4,
        playlistSize: options?.parameters?.playlistSize as number || this.defaultStreamSettings.playlistSize || 5,
        outputDir,
        streamId,
        ...
        videoQualities: selectedQualities as any,
        security: {
          enableEncryption: options?.parameters?.enableEncryption as boolean || 
                            (this.defaultStreamSettings.security?.enableEncryption || false),
          keyRotationInterval: options?.parameters?.keyRotationInterval as number || 
                              this.defaultStreamSettings.security?.keyRotationInterval
        },
        lowLatency: {
          enabled: options?.parameters?.lowLatency as boolean || 
                  (this.defaultStreamSettings.lowLatency?.enabled || false),
          partDuration: options?.parameters?.partDuration as number || 
                      this.defaultStreamSettings.lowLatency?.partDuration,
          preloadHints: options?.parameters?.preloadHints as number || 
                      this.defaultStreamSettings.lowLatency?.preloadHints
        },
        cdn: {
          enabled: options?.parameters?.cdnEnabled as boolean || 
                  (this.defaultStreamSettings.cdn?.enabled || false),
          baseUrl: options?.parameters?.cdnBaseUrl as string || 
                  this.defaultStreamSettings.cdn?.baseUrl,
          cacheControl: options?.parameters?.cacheControl as string || 
                       this.defaultStreamSettings.cdn?.cacheControl
        },
        ffmpegOptions: options?.parameters?.ffmpegOptions as string[] || 
      ...
      this.logger.error(`Failed to start HLS stream: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  private async startFFmpegProcess(
    ...
    for (let i = 0; i < qualityVariants.length; i++) {
      ...
      baseArgs.push(
        ...
        '-g', `${2 * Math.round(30)}`, // GOP size (2 seconds at 30 fps)
        '-keyint_min', `${Math.round(30)}`, // Minimum keyframe interval
        '-r', '30', // Frame rate
        '-b:v', `${quality.bitrate}`, // Video bitrate
        '-maxrate', `${Math.floor(quality.bitrate * 1.2)}`, // Max bitrate
        '-bufsize', `${Math.floor(quality.bitrate * 2)}`, // Buffer size
      ...
      baseArgs.push(
        '-f', 'hls', // Format
        '-hls_time', `${settings.segmentDuration}`, // Segment duration
        '-hls_list_size', `${settings.playlistSize}`, // Playlist size
        '-hls_segment_filename', `${path.join(variantDir, 'segment_%03d.ts')}`, // Segment filename pattern
      ...
      if (settings.lowLatency?.enabled) {
        baseArgs.push(
          '-hls_flags', 'program_date_time+low_latency',
          '-hls_segment_type', 'fmp4', // Use fMP4 segments for LL-HLS
          '-hls_fmp4_init_filename', 'init.mp4', // Initialization segment name
          '-hls_segment_options', 'fmp4_padding=1', // Add empty moof atom for low latency
          '-hls_playlist_type', 'event', // Live event
          '-method', 'PUT' // Use PUT for segment uploads
        );
        
        if (settings.lowLatency.partDuration) {
          baseArgs.push(
            '-hls_part_time', `${settings.lowLatency.partDuration}` // Part duration for LL-HLS
          );
        }
      }
        ...
        ffmpeg.stderr.on('data', (data) => {
          ...
          if (output.includes('Opening') && output.includes('for writing')) {
            ...
            if (match && match[1]) {
              ...
              const variantName = variantIndex < pathParts.length ? pathParts[variantIndex] : 'unknown';
  ...
  private async generateMasterPlaylist(streamId: string, streamInfo: HLSStreamInfo): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Error generating master playlist: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
   ...
   * @param settings Stream settings
   */
  private async generateEncryptionKeys(streamId: string, settings: HLSStreamSettings): Promise<void> {
    if (!settings.security?.enableEncryption) {
      return;
    }
    ...
    try {
      ...
      this.logger.error(`Error generating encryption keys: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  private async stopHLSStream(streamId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Error stopping HLS stream: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
