...
export class ServiceCommunicationClient {
  ...
  private recordFailure(): void {
    if (this.circuitState === CircuitState.HALF_OPEN) {
      ...
      this.halfOpenRequests = 0;
      this.logger.error(`Circuit is open again for ${this.options.baseUrl} until ${new Date(this.nextAttempt).toISOString()}`);
    } else if (this.circuitState === CircuitState.CLOSED) {
      this.failureCount++;
      ...
      if (this.failureCount >= this.circuitBreakerOptions.failureThreshold) {
        this.circuitState = CircuitState.OPEN;
        this.nextAttempt = Date.now() + this.circuitBreakerOptions.resetTimeout;
        this.logger.error(`Circuit is now open for ${this.options.baseUrl} until ${new Date(this.nextAttempt).toISOString()}`);
      }
    }
  }
  ...
  private async executeWithRetry<T>(
    ...
    while (true) {
      try {
        ...
        this.logger.debug(`Retrying ${method} ${url} after ${delay}ms (${retries + 1}/${maxRetries})`);
  ...
  public getCircuitState(): {
    ...
    return {
      ...
      nextAttempt: this.circuitState === CircuitState.OPEN 
