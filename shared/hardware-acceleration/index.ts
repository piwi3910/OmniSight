...
export interface AccelerationConfig {
  ...
  taskSpecificPlatforms?: {        // Platform overrides for specific tasks
  ...
  customParameters?: { [key: string]: any }; // Platform-specific parameters
}
...
export interface AccelerationProvider {
   ...
   */
  executeTask<TInput, TOutput>(task: AccelerationTask<TInput, TOutput>): Promise<AccelerationResult<TOutput>>;
...
export class AccelerationProviderFactory {
   ...
   */
  public static registerProvider(platform: AccelerationPlatform, factory: () => AccelerationProvider): void {
...
export class AccelerationManager {
   ...
   */
  public async getAvailableCapabilities(): Promise<Map<AccelerationPlatform, AccelerationCapability[]>> {
    if (!this.initialized) {
   ...
   */
  private async getBestProviderForTask(taskType: AccelerationTaskType, preferredPlatform?: AccelerationPlatform): Promise<AccelerationProvider | null> {
    if (!this.initialized) {
      await this.initialize();
    }
    ...
    // 4. Fallback order
    const platformToUse = preferredPlatform || taskSpecificPlatform || this.config.preferredPlatform;
    ...
    if (platformToUse) {
      ...
      if (provider) {
        ...
        const hasCapability = capabilities.some(cap => 
          cap.taskTypes.includes(taskType) && cap.supported
        );
   ...
   */
  public async executeTask<TInput, TOutput>(task: AccelerationTask<TInput, TOutput>): Promise<AccelerationResult<TOutput>> {
    if (!this.initialized) {
      await this.initialize();
    }
...
export const AccelerationUtils = {
   ...
   */
  getBestPlatformForTask: async (taskType: AccelerationTaskType): Promise<AccelerationPlatform | null> => {
    const manager = AccelerationManager.getInstance();
    const capabilities = await manager.getAvailableCapabilities();
